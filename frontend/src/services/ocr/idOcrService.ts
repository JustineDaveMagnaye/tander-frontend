...
/**
 * Generic ID document keywords - must find at least some of these
 * to confirm the image is actually an ID document
 */
const GENERIC_ID_KEYWORDS = [
  // Document type indicators
  'REPUBLIC', 'PHILIPPINES', 'PILIPINAS', 'REPUBLIKA',
  'IDENTIFICATION', 'ID', 'CARD', 'CERTIFICATE',

  // Personal info fields
  'NAME', 'PANGALAN', 'FIRST NAME', 'LAST NAME', 'MIDDLE NAME',
  'SURNAME', 'GIVEN NAME', 'APELYIDO',
  'DATE OF BIRTH', 'DOB', 'BIRTH', 'BIRTHDAY', 'KAPANGANAKAN', 'KAARAWAN',
  'SEX', 'GENDER', 'KASARIAN',
  'ADDRESS', 'TIRAHAN', 'RESIDENCE',

  // ID-specific terms
  'EXPIRY', 'EXPIRATION', 'VALID', 'VALIDITY', 'ISSUED',
  'SIGNATURE', 'PIRMA', 'LAGDA',
  'PHOTO', 'PICTURE',
  'NUMBER', 'NO.', 'NUMERO',

  // Government/issuing authority
  'GOVERNMENT', 'PAMAHALAAN', 'OFFICE', 'DEPARTMENT', 'BUREAU',
  'CITY', 'LUNGSOD', 'MUNICIPALITY', 'BARANGAY', 'PROVINCE',
];

/**
 * Minimum requirements to consider an image as a valid ID
 */
const MIN_ID_KEYWORD_MATCHES = 3;  // Must match at least 3 generic keywords
const MIN_TEXT_LENGTH = 50;        // ID should have reasonable amount of text

/**
 * Image quality thresholds for blur detection
 */
const QUALITY_THRESHOLDS = {
  MIN_WORD_LENGTH_AVG: 2.5,        // Average word length (blurry = fragmented single chars)
  MIN_ALPHANUMERIC_RATIO: 0.6,    // Ratio of letters/numbers to total chars
  MIN_READABLE_WORDS: 5,          // Minimum words with 3+ characters
  MAX_GARBAGE_RATIO: 0.3,         // Max ratio of single-char "words"
};

/**
 * Detect if image appears blurry based on OCR output quality.
 * Blurry images produce:
 * - Fragmented text (many single characters)
 * - High noise ratio (random symbols)
 * - Few recognizable words
 */
function detectImageQualityIssues(text: string): {
  isBlurry: boolean;
  ...
  if (!text || text.length === 0) {
    return {
      ...
      isLowQuality: true,
      qualityScore: 0,
      issues: ['No text could be extracted - image may be too blurry or not an ID'],
    };
  }
  ...
  if (totalWords === 0) {
    return {
      ...
      isLowQuality: true,
      qualityScore: 0,
      issues: ['No readable words found'],
    };
  }
  ...
  const garbageRatio = singleCharWords / totalWords;
  const readableWords = words.filter(w => w.length >= 3).length;
  ...
  console.log(`[OCR Quality] Words: ${totalWords}, AvgLen: ${avgWordLength.toFixed(1)}, ` +
    `Readable: ${readableWords}, GarbageRatio: ${(garbageRatio * 100).toFixed(0)}%, ` +
    `AlphaNum: ${(alphanumericRatio * 100).toFixed(0)}%`);

  // Check each quality metric
  let qualityScore = 100;

  if (avgWordLength < QUALITY_THRESHOLDS.MIN_WORD_LENGTH_AVG) {
    issues.push('Text appears fragmented (possible blur)');
    qualityScore -= 25;
  }

  if (garbageRatio > QUALITY_THRESHOLDS.MAX_GARBAGE_RATIO) {
    issues.push('Too many single characters detected (image may be blurry)');
    qualityScore -= 30;
  }

  if (readableWords < QUALITY_THRESHOLDS.MIN_READABLE_WORDS) {
    issues.push('Few readable words found (check focus and lighting)');
    qualityScore -= 25;
  }

  if (alphanumericRatio < QUALITY_THRESHOLDS.MIN_ALPHANUMERIC_RATIO) {
    issues.push('High noise ratio in text (image quality issue)');
    qualityScore -= 20;
  }

  qualityScore = Math.max(0, qualityScore);

  const isBlurry = qualityScore < 50 || garbageRatio > 0.4 || avgWordLength < 2;
...
/**
 * Validate that the extracted text appears to be from an actual ID document
 * Returns a validation result with score and reasons
 */
function validateIsIdDocument(text: string, idType: PhilippineIDType): {
  isValid: boolean;
  ...
  if (text.length < MIN_TEXT_LENGTH) {
    return {
      ...
      confidence: 'NONE',
      matchedKeywords: [],
      reason: `Text too short (${text.length} chars). ID documents have more text.`,
    };
  }

  // If we detected a specific ID type, that's high confidence
  if (idType !== 'unknown') {
    return {
      isValid: true,
      confidence: 'HIGH',
      matchedKeywords: ID_TYPE_KEYWORDS[idType].filter(kw => upperText.includes(kw)),
      reason: `Detected ${idType.replace('_', ' ')} ID type`,
    };
  }

  // For unknown ID type, check generic keywords
  const matchedKeywords = GENERIC_ID_KEYWORDS.filter(kw => upperText.includes(kw));
  const matchCount = matchedKeywords.length;

  console.log(`[OCR] ID validation: ${matchCount} keywords matched:`, matchedKeywords);

  // Must have DOB-related keyword to extract age
  const hasDobKeyword = /DATE\s*OF\s*BIRTH|DOB|BIRTH|BIRTHDAY|KAPANGANAKAN|KAARAWAN/i.test(upperText);

  if (matchCount >= MIN_ID_KEYWORD_MATCHES && hasDobKeyword) {
    return {
      isValid: true,
      confidence: matchCount >= 6 ? 'HIGH' : 'MEDIUM',
      matchedKeywords,
      reason: `Matched ${matchCount} ID keywords including DOB field`,
    };
  }

  if (matchCount >= MIN_ID_KEYWORD_MATCHES) {
    // Has some ID keywords but no DOB field visible
    return {
      isValid: false,
      confidence: 'LOW',
      matchedKeywords,
      reason: 'Found ID keywords but no date of birth field detected',
    };
  }

  // Not enough evidence this is an ID
  return {
    isValid: false,
    confidence: 'NONE',
    matchedKeywords,
    reason: `Only ${matchCount} ID keywords found (need ${MIN_ID_KEYWORD_MATCHES}). This doesn't appear to be an ID document.`,
  };
}
...
export async function extractDOBFromID(
  ...
  try {
    ...
    // CHECK IMAGE QUALITY - detect blur before proceeding
    const qualityCheck = detectImageQualityIssues(rawText);
    console.log('[OCR] Image quality check:', qualityCheck);

    if (qualityCheck.isBlurry) {
      console.log('[OCR] REJECTED: Image appears blurry or low quality');
      return {
        success: false,
        dateOfBirth: null,
        expirationDate: null,
        ...
        meetsAgeRequirement: false,
        isExpired: false,
        errorMessage: `Image appears blurry or out of focus. ${qualityCheck.issues[0] || 'Please take a clearer photo.'}`,
      };
    }

    if (qualityCheck.isLowQuality) {
      console.log('[OCR] WARNING: Low quality image detected, may have issues');
      // Continue but log warning - let ID validation decide
    }
    ...
    // VALIDATION: Check if this actually looks like an ID document
    const idValidation = validateIsIdDocument(rawText, idType);
    console.log('[OCR] ID validation result:', idValidation);

    if (!idValidation.isValid) {
      console.log('[OCR] REJECTED: Image does not appear to be a valid ID document');
      return {
        success: false,
        dateOfBirth: null,
        expirationDate: null,
        ...
        meetsAgeRequirement: false,
        isExpired: false,
        errorMessage: idValidation.reason || 'This does not appear to be a valid ID document. Please scan a Philippine government-issued ID.',
      };
    }
