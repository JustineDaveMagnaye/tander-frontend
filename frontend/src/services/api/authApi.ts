...
// Comparison result from backend (includes all 3 age sources)
export interface OcrComparisonResult {
  frontendAge: number | null;
  backendAge: number;
  cloudflareAge: number | null;  // NEW: Cloudflare AI extracted age
  cloudflareDob: string | null;  // NEW: Cloudflare extracted DOB (YYYY-MM-DD)
  cloudflareAgeReadable: boolean;  // NEW: Whether Cloudflare could read DOB
  agesMatch: boolean;
...
// Fraud analysis result from Cloudflare Workers AI (with age extraction)
export interface FraudAnalysisResult {
  ...
  auditId?: string;
  skipReason?: string;
  // Age extraction fields (Cloudflare as second opinion)
  extractedDob?: string | null;      // DOB extracted by Cloudflare (YYYY-MM-DD)
  extractedAge?: number | null;      // Age calculated from Cloudflare DOB
  dobReadable?: boolean | null;      // Whether Cloudflare could read the DOB
  dobConfidence?: number | null;     // Confidence in DOB extraction (0.0-1.0)
}
