...
import { DataPrivacyModal } from '@shared/components/DataPrivacyModal';
...
export const LoginScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const [pendingBiometricCreds, setPendingBiometricCreds] = useState<{ username: string; password: string } | null>(null);
  ...
  }, [authLogin, password, routeAfterLogin, username]);
  ...
  const handleLogin = useCallback(async () => {
    ...
    // If terms or privacy not agreed, show the agreement required modal
    ...
    await performLogin();
  }, [agreedToTerms, agreedToPrivacy, clearErrors, loading, performLogin, validate]);
  ...
  const handleAgreeAndContinue = useCallback(async () => {
    ...
    // Check if we have pending biometric credentials
    if (pendingBiometricCreds) {
      setLoading(true);
      try {
        await authLogin(pendingBiometricCreds.username, pendingBiometricCreds.password);
        routeAfterLogin(pendingBiometricCreds.username);
      } catch (err: any) {
  ...
  }, [authLogin, pendingBiometricCreds, performLogin, routeAfterLogin]);

  const handleDeclineAgreement = useCallback(() => {
    setShowAgreementRequired(false);
    setPendingBiometricCreds(null);
  }, []);
  ...
  const handleBiometric = useCallback(async () => {
    ...
    try {
      ...
      // If terms or privacy not agreed, show the agreement required modal
      if (!agreedToTerms || !agreedToPrivacy) {
        setPendingBiometricCreds(creds);
        setShowAgreementRequired(true);
        return;
      }
  ...
  }, [agreedToTerms, agreedToPrivacy, authenticateAndGetCredentials, authLogin, biometricAuthenticating, clearErrors, clearBiometricError, loading, routeAfterLogin]);
  ...
  const handleCreateAccount = useCallback(async () => {
    ...
    try {
      ...
      console.log('[LoginScreen] Loaded verification config - Minimum age:', minimumAge);
  ...
  return (
    <View style={styles.container}>
              ...
              >
                ...
                <View style={styles.agreementSection}>
                  ...
                  >
                    ...
                    <View style={styles.termsCheckboxTextContainer}>
                      <Text style={styles.termsCheckboxText} maxFontSizeMultiplier={FONT_SCALING.CAPTION}>
                        I agree to the{' '}
                      </Text>
                      <TouchableOpacity onPress={() => setShowTerms(true)}>
                        <Text style={styles.termsCheckboxLink} maxFontSizeMultiplier={FONT_SCALING.CAPTION}>
                          Terms & Conditions
                        </Text>
                      </TouchableOpacity>
                    </View>
                  ...
                  <TouchableOpacity
                    ...
                    accessibilityLabel="I agree to the Data Privacy Policy"
                  >
                    <View style={[styles.checkbox, agreedToPrivacy && styles.checkboxCheckedOrange]}>
                      {agreedToPrivacy && (
                        <Feather name="check" size={14} color="#FFFFFF" />
                      )}
                    </View>
                    <View style={styles.termsCheckboxTextContainer}>
                      ...
                      <TouchableOpacity onPress={() => setShowPrivacy(true)}>
                        <Text style={styles.privacyCheckboxLink} maxFontSizeMultiplier={FONT_SCALING.CAPTION}>
                          Data Privacy Policy
                        </Text>
                      </TouchableOpacity>
                    </View>
      ...
      <Modal
        visible={showAgreementRequired}
        transparent
        animationType="fade"
        onRequestClose={handleDeclineAgreement}
      >
        <View style={styles.termsModalOverlay}>
          <View style={styles.termsModalContent}>
            {/* Icon */}
            <View style={styles.termsModalIconContainer}>
              <LinearGradient
                colors={[THEME.accent.teal, THEME.accent.tealDark]}
                style={styles.termsModalIconGradient}
              >
            ...
            {/* Title */}
            <Text style={styles.termsModalTitle} maxFontSizeMultiplier={FONT_SCALING.TITLE}>
              Agreement Required
            </Text>

            {/* Message */}
            <Text style={styles.termsModalMessage} maxFontSizeMultiplier={FONT_SCALING.BODY}>
              To continue using Tander, you must agree to our Terms & Conditions and Data Privacy Policy.
            </Text>

            {/* View Links */}
            <View style={styles.agreementLinksContainer}>
              <TouchableOpacity
                style={styles.termsModalViewLink}
                onPress={() => {
                  setShowAgreementRequired(false);
                  setTimeout(() => setShowTerms(true), 300);
                }}
              >
                <Feather name="file-text" size={16} color={THEME.accent.teal} />
                <Text style={styles.termsModalViewLinkText} maxFontSizeMultiplier={FONT_SCALING.CAPTION}>
                  Terms & Conditions
                </Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={[styles.termsModalViewLink, styles.privacyViewLink]}
                onPress={() => {
                  setShowAgreementRequired(false);
                  setTimeout(() => setShowPrivacy(true), 300);
                }}
              >
                <Feather name="lock" size={16} color={THEME.accent.orange} />
                <Text style={styles.privacyModalViewLinkText} maxFontSizeMultiplier={FONT_SCALING.CAPTION}>
                  Data Privacy Policy
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.termsModalButtons}>
              {/* Decline Button */}
              <TouchableOpacity
                style={styles.termsModalDeclineBtn}
                onPress={handleDeclineAgreement}
                activeOpacity={0.7}
              >
                <Text style={styles.termsModalDeclineText} maxFontSizeMultiplier={FONT_SCALING.BUTTON}>
                  Decline
                </Text>
              </TouchableOpacity>
              ...
              {/* Agree Button */}
              <TouchableOpacity
                style={styles.termsModalAgreeBtn}
                onPress={handleAgreeAndContinue}
                activeOpacity={0.85}
                disabled={loading}
              >
                <LinearGradient
                  colors={[THEME.accent.orange, THEME.accent.orangeDark]}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
                  style={styles.termsModalAgreeBtnGradient}
                >
                  {loading ? (
                    ...
                    <>
                      ...
                      <Text style={styles.termsModalAgreeText} maxFontSizeMultiplier={FONT_SCALING.BUTTON}>
                        I Agree to Both
                      </Text>
                    </>
                  )}
            ...
            {/* Warning */}
            <View style={styles.termsModalWarning}>
              <Feather name="info" size={14} color={THEME.text.light} />
              <Text style={styles.termsModalWarningText} maxFontSizeMultiplier={FONT_SCALING.CAPTION}>
...
const styles = StyleSheet.create({
  ...
  termsModalIconContainer: {
    marginBottom: 20,
  },
  termsModalIconGradient: {
    width: 72,
    height: 72,
    borderRadius: 36,
    justifyContent: 'center',
  ...
  termsModalViewLinkText: {
    fontSize: 14,
    fontWeight: '600',
    color: THEME.accent.teal,
  },
  privacyModalViewLinkText: {
    fontSize: 14,
    fontWeight: '600',
    color: THEME.accent.orange,
  },
