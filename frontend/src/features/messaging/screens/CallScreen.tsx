  ...
  useEffect(() => {
    if ((callState === 'ended' || callState === 'failed') && !isNavigatingAwayRef.current) {
      ...
      const timeoutId = setTimeout(() => {
        if (isMountedRef.current) {
          try {
            // Use replace to remove CallScreen from stack, preventing back navigation to it
            navigation.dispatch(
      ...
      <Animated.View style={[styles.container, { opacity: fadeIn }]}>
        ...
        {isLandscape ? (
          ...
          >
            ...
            <View style={[styles.audioControlsLandscape, { gap: sizes.sectionGap }]}>
              {isConnected && (
          ...
          >
            ...
            {/* Controls */}
            <View style={[styles.audioControls, { gap: sizes.sectionGap }]}>
              {isConnected && (
  ...
  return (
    <TouchableWithoutFeedback onPress={handleScreenPress}>
      <Animated.View style={[styles.container, { opacity: fadeIn }]}>
        ...
        {/* Remote Video */}
        {!isVideoMuted && isConnected && remoteStreamUrl ? (
          <RTCView
        ...
        {!isVideoMuted && localStreamUrl && (
          ...
          >
            <RTCView
              key={`self-${isFrontCamera ? 'front' : 'back'}`}
            ...
            <View style={styles.pipLabel}>
              <Text style={[styles.pipLabelText, { fontSize: sizes.labelFont - 2 }]}>You</Text>
            </View>
          </View>
        )}
