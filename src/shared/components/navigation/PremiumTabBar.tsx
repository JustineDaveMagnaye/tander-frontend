...
// Tab configuration
const TAB_CONFIG: Record<string, { icon: string; label: string; isCenter?: boolean }> = {
  MessagesTab: { icon: 'message-circle', label: 'Messages' },
  MatchesTab: { icon: 'heart', label: 'Matches' },
  DiscoveryTab: { icon: 'tander-logo', label: 'Discover', isCenter: true },
  ProfileTab: { icon: 'user', label: 'Profile' },
  TandyTab: { icon: 'headphones', label: 'Tandy' },
};
...
const styles = StyleSheet.create({
  ...
  blurContainer: {
    ...
    backgroundColor: Platform.OS === 'android' ? 'rgba(255,255,255,0.98)' : 'rgba(255,255,255,0.9)',
  },
  topBorderGradient: {
    position: 'absolute',
    top: 0,
